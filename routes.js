...
//2. GET NEW
  ...
  app.post("/payment/full", isLoggedIn, function (req, res) {
    ...
    payment(paidExpense, receivedExpense)
  ...
  //make payment
  function payment(paidExpense, receivedExpense){
    ...
    Archive.create(paidExpense, function (err, newlyCreated) {
      if (err) {
        console.log(err);
      } else {
    ...
    Archive.create(receivedExpense, function (err, newlyCreated) {
      if (err) {
        console.log(err);
      } else {
        console.log('archived expense created:' + newlyCreated);
